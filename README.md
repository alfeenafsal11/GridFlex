...
As distribution grids incorporate increasing levels of variable renewable energy (VRE), distribution system operators (DSOs) experience steep ramp rates, localized substation congestion, and pronounced evening demand peaks. While battery energy storage systems (BESS) offer fast-acting flexibility, this work uses a simple surplus-following reactive controller (which only actuates when local solar generation exceeds local load) as a deliberately myopic baseline. In low-solar seasonal conditions, such reactive heuristics can remain almost entirely idle, failing to alleviate grid peaks or capture economic value.
...
2. **The Reactive Inaction of Surplus-Following Heuristics**:
   During winter, solar generation (159.4 MWh) is small compared to consumer load (2,819.6 MWh), and local solar power rarely exceeds local demand. Under the surplus-following rule-based controller, the battery discharges its initial energy in the first two hours and remains pinned at minimum SOC (0.10) for 1,288 out of 1,290 hours (99.845%, or approximately 99.85% idle time), delivering 0.00% peak demand reduction.
3. **Forecasting Horizon Dynamics**:
   LightGBM achieves strong error reduction at the immediate lookahead horizon $h=1$ (-57.05% for load: 47.06 kW vs 109.56 kW persistence; -21.59% for solar: 67.59 kW vs 86.20 kW persistence). Beyond $h=1$, error does not degrade monotonically: load error rises to ~120 kW by $h=5$ and plateaus with diurnal variations up to 135.91 kW at $h=24$ (where persistence is 109.95 kW), while solar error fluctuates between ~240 kW and ~292 kW before ending at 265.52 kW at $h=24$ (where persistence is 85.57 kW). Because receding-horizon MPC actuates only the immediate $h=1$ action before reforecasting, control operates continuously in the high-accuracy regime. Injected forecast perturbations show that dispatch quality is sensitive to forecast error: sufficiently large noise (20%–30%) can cause the simulated controller to charge during peak periods, worsening peak demand relative to the no-battery baseline.
